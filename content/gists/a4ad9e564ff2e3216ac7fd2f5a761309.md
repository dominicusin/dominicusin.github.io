...
$usernameexp = "uefault";
$passwordexp = "zfghjsrfghj";
$usernameimp = "uflarum";
$passwordimp = "rghaergh";
$exportDBName = "udefault";
$importDBName = "uflarum";
$exportDBPrefix = "phpbb_";
$importDBPrefix = "Flarum"; // Leave blank if you did not supply a table prefix when setting up Flarum. Otherwise, supply the full table prefix (i.e. including "_", if used).


// Establish a connection to the server where the PHPBB database exists
$exportDbConnection = new mysqli($servername, $usernameexp, $passwordexp, $exportDBName);
...
$importDbConnection = new mysqli($servername, $usernameimp, $passwordimp, $importDBName);
...
if ($totalUsers)
{
	...
	while($row = $result->fetch_assoc())
	{
		...
		if($row["user_email"] != NULL)
		{
			...
			$query = "INSERT IGNORE INTO " . $importDBPrefix . "users (id, username, email, password, joined_at, is_email_confirmed) VALUES ( '$id', '$formatedUsername', '$email', '$password', '$jointime', 1)";
...
if ($totalCategories)
{
	...
	while($row = $result->fetch_assoc())
	{
		...
		$query = "INSERT IGNORE INTO " . $importDBPrefix . "tags (id, name, description, slug, color, position) VALUES ( '$id', '$name', '$description', '$slug', '$color', '$position')";
		$res = $importDbConnection->query($query);
		if($res === false) {
...
{
	$curTopicCount = 0;
	$insertString = "INSERT IGNORE INTO " . $importDBPrefix . "posts (id, user_id, discussion_id, created_at, type, content) VALUES \n";
	...
	while($topic = $topicsQuery->fetch_assoc())
	{
		...
		$query = "INSERT IGNORE INTO " . $importDBPrefix . "discussion_tag (discussion_id, tag_id) VALUES( '$topicid', '$forumid')";
		$res = $importDbConnection->query($query);
		if($res === false) {
			echo "Wrong SQL: " . $query . " Error: " . $importDbConnection->error . " <br/>\n";
		}
		...
		if($result['parent_id'] > 0){
			...
			$query = "INSERT IGNORE INTO " . $importDBPrefix . "discussion_tag (discussion_id, tag_id) VALUES( '$topicid', '$parentid')";
			$res = $importDbConnection->query($query);
			if($res === false) {
				echo "Wrong SQL: " . $query . " Error: " . $importDbConnection->error . " <br/>\n";
			}
		...
		$query = "INSERT IGNORE INTO " . $importDBPrefix . "discussions (id, title, slug, created_at, comment_count, participant_count, first_post_id, last_post_id, user_id, last_posted_user_id, last_posted_at) VALUES( '$topicid', '$topicTitle', '$slug', '$discussionDate', '$postCount', '$count', 1, 1, '$poster', '$lastPosterID', '$discussionDate')";
		$res = $importDbConnection->query($query);
		if($res === false) {
			echo "Wrong SQL: " . $query . " Error: " . $importDbConnection->error . " <br/>\n";
		}
...
if ($result->num_rows > 0)
{
	...
	while($row = $result->fetch_assoc()) {
		...
		$query = "INSERT IGNORE INTO " . $importDBPrefix . "discussion_user (user_id, discussion_id) VALUES ( '$userID', '$topicID')";
		$res = $importDbConnection->query($query);
		if($res === false) {
			echo "Wrong SQL: " . $query . " Error: " . $importDbConnection->error . " <br/>\n";
		}
