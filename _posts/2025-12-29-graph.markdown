---
title: Graph
date: 2025-12-29 21:51:00 +02:00
---
...
<head>
    ...
    <title>Граф преобразований форматов</title>
...
<body>
    <div id="controls">
        <h3>🔍 Фильтры</h3>
        <input type="text" id="search" placeholder="Поиск формата...">
        <label><input type="checkbox" id="showMarkup" checked> Языки разметки</label>
        <label><input type="checkbox" id="showTeX" checked> TeX/LaTeX системы</label>
        <label><input type="checkbox" id="showOffice" checked> Офисные форматы</label>
        <label><input type="checkbox" id="showEbook" checked> Электронные книги</label>
        <label><input type="checkbox" id="showScientific" checked> Научные форматы</label>
        <label><input type="checkbox" id="showOutput" checked> Выходные форматы</label>
    </div>
    ...
    <div id="legend">
        ...
        <div class="legend-item">
            ...
            <span>Научные форматы</span>
        </div>
        <div class="legend-item">
            <div class="legend-color" style="background: #4dd0e1;"></div>
            <span>Выходные форматы</span>
        </div>
    </div>
    ...
    <script>
        ...
        // Определение рёбер (преобразований)
        const edges = new vis.DataSet([
            // Pandoc конвертации (основной инструмент)
        ...
        // Информация о формате при клике
        network.on("click", function(params) {
            if (params.nodes.length > 0) {
                ...
                let html = `<p><strong>Конвертация из ${node.label}:</strong></p><ul>`;
                outEdges.forEach(e => {
                ...
                html += `<p><strong>Конвертация в ${node.label}:</strong></p><ul>`;
                inEdges.forEach(e => {
